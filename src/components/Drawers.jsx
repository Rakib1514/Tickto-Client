      ...
      <div className="h-full flex flex-col bg-white">
        ...
        <div className="p-4 border-b">
          <div className="flex gap-3 items-center mb-3">
            <img
              src={selectedTrip?.busDetails?.images?.[0] || "https://i.ibb.co/fHRKw9v/emad.png"}
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">{selectedTrip?.busDetails?.brand}</p>
              <p className="text-xs text-gray-400">Reg: #{selectedTrip?.busDetails?.registrationNumber}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4">
          {selectedTrip?.busDetails?.seatLayout?.map((row) => (
            <div key={row.row} className="flex gap-2 items-center mb-3">
              <span className="w-6 text-right font-medium">{row.row}</span>
              <div className="flex gap-2 flex-wrap">
                {row.seats.map((seat) => {
                  ...
                  return (
                    <button
                      key={seatNum}
                      className={`w-10 h-10 rounded-full flex items-center justify-center border font-semibold text-sm transition
                        ${
                          isBooked
